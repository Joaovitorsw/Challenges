...
let app, getByTestId, queryByTestId, getByText, availablePlayersTableBody, selectedPlayersTableBody;
beforeEach(() => {
  app = render(<App />);
...
it("Select first player in Available Players", () => {
  let playerButton = getByTestId(`available-${players[0]["name"].split(" ").join("-")}-select`);
  fireEvent.click(playerButton);
  expect(selectedPlayersTableBody.children.length).toBe(1);
  expect(selectedPlayersTableBody.children[0].children[0].textContent).toBe(players[0]["name"]);
...
it("Remove Selected player", () => {
  ...
  let playerRemoveButton = getByTestId(`selected-${players[0]["name"].split(" ").join("-")}-remove`);
...
it("Select 5 players", () => {
  for (let i = 0; i < 5; i++) {
    let playerSelectButton = getByTestId(`available-${players[i]["name"].split(" ").join("-")}-select`);
    fireEvent.click(playerSelectButton);
  }
  expect(selectedPlayersTableBody.children.length).toBe(5);
  for (let i = 0; i < 5; i++) {
    expect(selectedPlayersTableBody.children[i].children[0].textContent).toBe(players[i]["name"]);
  }
});
...
it("Add 5 players and remove them", () => {
  ...
  expect(selectedPlayersTableBody.children.length).toBe(5);
  for (let i = 0; i < 5; i++) {
    let playerRemoveButton = getByTestId(`selected-${players[i]["name"].split(" ").join("-")}-remove`);
    fireEvent.click(playerRemoveButton);
...
it("Check limit on adding more than 6 batsman", () => {
  ...
  let playerSelectButton = getByTestId(`available-${players[6]["name"].split(" ").join("-")}-select`);
  fireEvent.click(playerSelectButton);
...
it("Check limit on adding more than 4 allrounders", () => {
  ...
  let playerSelectButton = getByTestId(`available-${players[15]["name"].split(" ").join("-")}-select`);
  fireEvent.click(playerSelectButton);
...
it("Check limit on adding more than 6 Bowlers", () => {
  ...
  let playerSelectButton = getByTestId(`available-${players[23]["name"].split(" ").join("-")}-select`);
  fireEvent.click(playerSelectButton);
